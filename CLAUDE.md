...
- `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`: enable per-caller rate limiting on the hosted path, counted in Upstash Redis over its REST API (no SDK). `RATE_LIMIT_PER_MIN` sets the fixed-window limit (default `120`; anything but a positive integer keeps the default). Unset = the limiter is the always-admit no-op, and a store outage admits the call rather than failing it. Set `MCP_CALLER_SALT` alongside them: it salts the hashed caller id in logs and rate-limit keys, and without it the Upstash token doubles as the salt, so rotating the token changes every caller id at once (restarting in-flight counters and breaking log correlation). Local stdio never rate-limits.
