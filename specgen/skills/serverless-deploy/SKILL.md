...
- Pin GPUs / set GPU count per worker — use `gpu.count`, `gpu.pools`, and `gpu.excludedTypes` on create/update, or the GPU-selection convenience tool.
- Read endpoint health and list workers — warmup and readiness, and the cold-start-vs-crash-loop signal (`initializing` vs `unhealthy`); worker status alone cannot rule a crash loop out, so pair it with the worker's logs.
...
- **CPU Serverless.** `create-endpoint` supports CPU workloads through `body.cpu`; read its schema and the CPU catalog for supported instance choices. Supply CPU or GPU configuration, never both.
...
| Multi-GPU / GPU-SKU pin | `set-endpoint-gpus` (`gpuCount`, `pools`) | `update-endpoint` (`gpu.count`, `gpu.pools`, `gpu.excludedTypes`) |
